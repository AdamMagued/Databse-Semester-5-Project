...
{
    public partial class CompLeaveApproval : Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    ...
                    // Check if it's compensation leave
                    SqlCommand checkCmd = new SqlCommand(
                        "SELECT COUNT(*) FROM Compensation_Leave WHERE request_ID = @id", conn);
                    checkCmd.Parameters.AddWithValue("@id", reqID);
                    int isCompLeave = (int)checkCmd.ExecuteScalar();

                    if (isCompLeave == 0)
                    {
                        Show("ERROR", "Not a compensation leave request,enter a compensation leave id", "rejected");
                        return;
                    }
